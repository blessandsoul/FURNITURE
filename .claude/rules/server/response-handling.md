...
- `code`: stable machine-readable string (e.g. `PRODUCT_NOT_FOUND`, `VALIDATION_FAILED`)
...
// Allowed
return reply.send(paginatedResponse("Products retrieved successfully", products, page, limit, totalCount));
```
...
```typescript
// Service returns:
return { items: products, totalItems: count };
...
| `NotFoundError` | 404 | `PRODUCT_NOT_FOUND` |
