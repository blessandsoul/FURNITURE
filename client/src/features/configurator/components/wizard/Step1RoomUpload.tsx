...
export function Step1RoomUpload(): React.JSX.Element {
    ...
    }, [t, uploadMutation, setRoomImage, autoSelectSofa]);
    ...
    return (
        <div className="space-y-6">
            <div>
                <h2 className="text-xl font-bold text-foreground">{t('roomUpload.heading')}</h2>
                <p className="mt-1 text-sm text-muted-foreground">
                    {t('roomUpload.subheading')}
                </p>
            </div>
            ...
            {hasImage ? (
                ...
                <div className="relative mx-auto max-w-lg overflow-hidden rounded-2xl border border-border/50 shadow-sm">
                    ...
                    <button
                        ...
                        aria-label={t('roomUpload.removeAria')}
                    >
                        <Trash className="h-4 w-4" />
                    </button>
                    <div className="absolute bottom-0 inset-x-0 bg-gradient-to-t from-black/40 to-transparent px-4 py-3">
                        <p className="text-xs font-medium text-white/90">{t('roomUpload.photoUploaded')}</p>
                    </div>
                </div>
            ) : (
                /* Upload zone */
                <button
                    ...
                    aria-label={t('roomUpload.uploadAria')}
                >
                    {uploadMutation.isPending ? (
                        <>
                            <SpinnerGap className="h-10 w-10 animate-spin text-primary" />
                            <p className="text-sm font-medium text-muted-foreground">{t('roomUpload.uploading')}</p>
                        </>
                    ) : (
                        <>
                            ...
                            <div className="text-center">
                                <p className="text-sm font-semibold text-foreground">
                                    {t('roomUpload.dragDrop')}
                                </p>
                                <p className="mt-1 text-xs text-muted-foreground">
                                    {t('roomUpload.orBrowse')}
                                </p>
                            </div>
                            <p className="text-[11px] text-muted-foreground/60">
                                {t('roomUpload.fileTypes')}
                            </p>
                        </>
                    )}
                </button>
