    ...
    if (generations.length === 0) {
        return (
            <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-border/50 bg-muted/10 px-6 py-16 text-center">
                ...
                <h3 className="text-sm font-medium text-foreground">{t('generation.emptyTitle')}</h3>
                <p className="mt-1 max-w-xs text-sm text-muted-foreground">
                    {t('generation.emptyBody')}
                </p>
            </div>
        );
    }
    ...
    return (
        <div className="space-y-4">
            <div className="flex items-center gap-2">
                <h2 className="text-lg font-semibold text-foreground">{t('generation.historyTitle')}</h2>
