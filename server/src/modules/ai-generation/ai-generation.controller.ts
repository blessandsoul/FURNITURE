...
import { GenerateSchema, AdminGenerationsFilterSchema } from './ai-generation.schemas.js';
...
class AiGenerationController {
  ...
  async listGenerations(request: FastifyRequest, reply: FastifyReply): Promise<void> {
    ...
    const { items, totalItems } = await aiGenerationService.getUserGenerations(userId, pagination);
    return reply.send(
